...
export default async function AssignmentPage({
  ...
  const { data: assignment } = await supabase
    .from("assignments")
    .select("id, title, instructions, due_date, max_points, requires_audio, classes(teacher_id)")
    .eq("id", assignmentId)
    ...
    <div className="mx-auto flex w-full max-w-2xl flex-1 flex-col gap-6 px-4 py-12">
      <div>
        ...
        <p className="text-muted-foreground">
          ...
          {assignment.requires_audio && (
            <>
              {" "}
              · <Badge variant="secondary">Audio required</Badge>
            </>
          )}
        </p>
      </div>
      ...
      {isStaffView ? (
        ...
        <StudentSubmission
          ...
          requiresAudio={assignment.requires_audio}
        />
      )}
    </div>
  );
...
async function StudentSubmission({
  ...
  const { data: submission } = await supabase
    .from("submissions")
    .select("id, content, file_path, file_name, audio_path, submitted_at, status, grades(points, feedback)")
    .eq("assignment_id", assignmentId)
  ...
  if (!submission) {
    return (
      <Card>
        ...
        <CardContent>
          <SubmitForm classId={classId} assignmentId={assignmentId} requiresAudio={requiresAudio} />
        </CardContent>
      </Card>
    );
  }
    ...
    <Card>
      ...
      <CardContent className="flex flex-col gap-4">
        ...
        {submission.audio_path && (
          <SpeakingRecordingPlayback path={submission.audio_path} label="Your spoken answer" />
        )}
          ...
          <>
            ...
            <div className="border-t border-border pt-4">
              ...
              <SubmitForm
                ...
                currentAudioPath={submission.audio_path}
